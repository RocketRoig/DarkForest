...
class Civilization:
    ...
    def _calculate_extinction_risk(self, total_energy_available):
        ...
        """
        scarcity_factor = max(0, 1 - self.energy_consumption/(total_energy_available if total_energy_available > 1 else 1))
        self.extinction_risk = (
            total_energy_available * math.exp(-(((1 - abs(self.random_gen.gauss(0, (scarcity_factor +1) / 30))) - 0) ** 2) /
                                         (2 * (( 1) / 8) ** 2)))
        self.extinction_risk = self.extinction_risk if self.extinction_risk>1e-3 else 0
    def _calculate_germination_event(self):
        """
        Calculates the extinction risk based on rapid growth during scarcity
        and incorporating dangers from the star system.
        """
        # Probability of triggering (e.g., 30% chance)
        genesis_probability = 1*10**-self.random_gen.uniform(4, 6) #/time unit
    ...
    def _calculate_growth_rate(self, total_energy_available):
        ...
        if self.energy_consumption < total_energy_available:
            abundance_factor = 1 - (self.energy_consumption / total_energy_available)
    ...
    def _update_energy_consumption(self, total_energy_available):
        ...
        self.energy_consumption = (((self.energy_consumption+1)**self.growth_rate)-1) - danger_impact - self.extinction_risk + self.germination_event
    ...
    def update(self):
        ...
        if self.kardashev_level >= 2:
            total_energy_available = (
                energy_budget['germination_planet_power'] +
                energy_budget['planets_power'] +
                energy_budget['star_energy_power']
            )
        elif self.kardashev_level == 1 :
            total_energy_available = (
                energy_budget['germination_planet_power'] +
                energy_budget['planets_power']
            )
        elif self.kardashev_level < 1:
            total_energy_available = energy_budget['germination_planet_power']
        else:
        ...
        self._calculate_germination_event()
...
if __name__ == "__main__":
    ...
    for global_time in range(time_steps):
        ...
        energy_consumptions.append(params['energy_consumption'])
        growth_rates.append(params['growth_rate'])
        kardashev_levels.append(params['kardashev_level'])
        extinction_risks.append(params['extinction_risk']/(params['extinction_risk'] if params['extinction_risk'] > 0 else 1))
        energy_available.append(params['energy_available'])
        star_energy_power.append(params['star_energy_power'])
        planets_power.append(params['planets_power'])
        germination_planet_power.append(params['germination_planet_power'])
        Star_System_Danger.append(params['Star_System_Danger']/(params['Star_System_Danger'] if params['Star_System_Danger'] > 0 else 1))
